### LL

**System Architect & Backend Engineer**
*Location: Guangzhou, CN*

Building scalable infrastructure and breaking digital barriers. Focus on high-load systems, hardware-level optimization, and distributed architecture.

---

### 🛠 Technical Arsenal

**Core Stack:**
![C#](https://img.shields.io/badge/C%23-%23239120.svg?style=for-the-badge&logo=c-sharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-5C2D91?style=for-the-badge&logo=.net&logoColor=white)
![C++](https://img.shields.io/badge/C++-%2300599C.svg?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/postgres-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white)

**Infrastructure & DevOps:**
![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Nginx](https://img.shields.io/badge/nginx-%23009639.svg?style=for-the-badge&logo=nginx&logoColor=white)

**Frontend & Integrations:**
![TypeScript](https://img.shields.io/badge/typescript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white)
![Telegram](https://img.shields.io/badge/Telegram_API-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)

---

### 🔭 Active Development

**Cargo.Solution** (Private Repo)
B2B SaaS ecosystem for international logistics.
- **Role:** Lead Architect.
- **Architecture:** Clean Architecture, Multi-tenancy (Global Query Filters), DDD.
- **Tech:** .NET 8, EF Core, PostgreSQL, React (Vite), JWT/Identity.
- **Status:** Production MVP deployed on Railway.

---

### 📊 Analytics

![Stats](https://github-readme-stats.vercel.app/api?username=GomenAmonai&show_icons=true&theme=graywhite&hide_border=true&count_private=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=GomenAmonai&layout=compact&theme=graywhite&hide_border=true&count_private=true)
---

> *"The only ones who should kill, are those who are prepared to be killed."*
